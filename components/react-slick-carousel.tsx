      ...
      <Slider {...settings}>
        {products.map((product, i) => (
          <Link key={product.id} href={`/product/${product.handle}`} className="flex flex-col items-center justify-center p-4 cursor-pointer">
            <div className="bg-black rounded-lg overflow-hidden flex items-center justify-center" style={{height: 320}}>
              <img src={product.featuredImage?.url} alt={product.title} className="object-contain w-full h-full" />
            </div>
